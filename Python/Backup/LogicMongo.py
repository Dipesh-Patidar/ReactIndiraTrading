...
@app.route('/optiondata', methods=["GET", "POST"])
def OptionData():
    ...
    if request.method == "GET":
        try:
            ...
            client = pymongo.MongoClient('192.168.100.22', 27017)
            ...
            result = pdf.to_json(orient="index")
...
@app.route('/optionperdata', methods=["GET", "POST"])
def OptionPerData():
    ...
    if request.method == "GET":
        ...
        result = FResult.to_json(orient="index")
        print("End: ",datetime.now())
        return result
...
def VixUpdate():
    global vdf
    FPath = "/home/pnp/Bhavcopy/IndiaVix" 
    CreFile = FPath +  "/*.csv"
...
def OptionIdxData():
    ...
    if request.method == "GET":
        ...
        FPath = "/home/pnp/Bhavcopy/Backup"
        ...
        pdf = pd.DataFrame(list(db["IOData"].find()))
        pdf = pdf.sort_values('Date')
        pdf.drop(["_id"],axis=1, inplace=True)
        pdf.reset_index(inplace=True)
        pdf.drop(["index"],axis=1, inplace=True)
