...
def ordertype():
    ...
    try:
        if request.method == "POST":
            ...
            # print("C: ",C)
            Od = {"SNO": [0], "Time": [" "], "NTStamp":[0], "OType": ["B"], "OrderN": ["NRML"], "Algo": ["SMP"], "MCount": [0], "SQ": ["No"],
            ...
            for x in range(0, C):
                ...
                Odf.loc[x] = [x, Pepoch, NTStamp, OT, ON, AO, MCount, SQ, SQCount, TOI, OID, TNQ, TQ, TQC, OP]
...
def PendingOrder():
    try:
        ...
        if request.method == "POST":
            ...
            POd = {"SNO":[0],"Time":[" "],"TStamp":[" "],"OType":["B"],"OrderN":["NRML"],"OrderID":[0],"TotalNQ":[0],"TQuantity":[0],"OCount":[0],"Price":[0]}
            ...
            for x in range(0,C):
                if ((ANOPdf.loc[x,"BS"])=="B") & ((ANOPdf.loc[x,"BS"])==Ndf.loc[0,"BS"]):
                    ...
                    if TNQ>TQ:
                        ...
                        POdf.loc[l] = [l,Pepoch,ANOPdf.loc[x,"TStamp"],OT,ON,TOI,TNQ,TQ,TQC,OP]
                        l = l+1
...
def StreamData():
    try:
        ...
        if request.method == "POST":
            ...
            #     dataf.append(name[(name["OType"]=="N")])
            #     df = pd.concat(dataf,ignore_index=True)
            #     print(df)
            ...
            #     if(C>=(LData-SData)):
            #         df = df.head((LData-SData))
            #         break


            C = df[df.columns[0]].count()

            SO = {"SNO": [0], "Time": [" "], "NTStamp":[" "], "OType": ["B"], "Token": [0], "OrderID": [0], "OID": [0], "TotalNQ": [0], "Price": [0]}
            ...
            for x in range(0,C):
                ...
                SOdf.loc[x] = [x,Pepoch,df.loc[x,"TStamp"],(df.loc[x,"BS"]), (df.loc[x,"Token"]), (df.loc[x,"Order"]),OID,(df.loc[x,"Qty"]),((df.loc[x,"Price"])/100)]
