...
def PendingOrder():
    try:
        ...
        if request.method == "POST":
            TokenNo = request.json['TokenNo']
            OrdID = float(request.json['OrderID'])
            ...
            NC =  (df["Order"]==OrdID) & (df["OType"]=="N") 
            BNC = (df["Order"]==OrdID) & (df["OType"]=="N") & (df["BS"]=="B") 
            SNC = (df["Order"]==OrdID) & (df["OType"]=="N") & (df["BS"]=="S")
            MC =  (df["Order"]==OrdID) & (df["OType"]=="M") 
            ...
            ANOP = (df["Order"]<OrdID) & (df["OType"]=="N") & (df["Price"]==Ndf.loc[0,"Price"])
            ...
            TC = (df["TStamp"]<Ndf.loc[0,"TStamp"]) & (df["OType"]=="T") & (df["Price"]==Ndf.loc[0,"Price"])
            Tdf = df[TC]

            # Pending New Order prior to Specific OID 

            POd = {"SNO":[0],"Time":[" "],"OType":["B"],"OrderN":["NRML"],"OrderID":[0],"TotalNQ":[0],"TQuantity":[0],"OCount":[0],"Price":[0]}
            ...
            for x in range(0,C):
                print((ANOPdf.loc[x,"BS"])," Ndf.loc[0,'BS']: ",Ndf.loc[0,"BS"])
                if ((ANOPdf.loc[x,"BS"])==Ndf.loc[0,"BS"]):
                    ...
                    if TNQ>TQ:
                        ...
                        POdf.loc[l] = [l,Pepoch,OT,ON,TOI,TNQ,TQ,TQC,OP]
                        l = l+1
