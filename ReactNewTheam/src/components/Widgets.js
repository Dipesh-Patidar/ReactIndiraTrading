...
export const CounterWidget = (props) => {
  const { icon, iconColor, category, title, period, percentage,percentageColor } = props;
  // const percentageIcon = percentage < 0 ? faAngleDown : faAngleUp;
  // const percentageColor = category === "Sell Traffic" ? "text-danger" : "text-success";
...
//   }
  ...
  return (
    <Card border="light" className="shadow-sm">
      <Card.Body>
        <Row className="d-block d-xl-flex align-items-center">
          <Col xs={6} xl={6} className="text-xl-center d-flex align-items-center justify-content-xl-center mb-3 mb-xl-0">
          
          </Col>
          <Col xs={12} xl={12} className="px-xl-0">
            <h5 className="mb-3 text-center">{title}</h5>
            {/* <table class="main-traffic">
            {Object.entries(data1).map(([keys,values]) => 
            { return (
             <> <tr class="trafic_share">
                <th class="inner-traffic">
             {/* <h6 key={`circle-element-`} className="fw-normal text-gray"> */}
                {/* <FontAwesomeIcon  className={`icon icon-xs text- w-20 me-1`} /> 
                <p>{`${keys}`}  </p> 
               {/* </h6> 
               </th>
               </tr>
               <tr class="trafic_value">
               <td></td>
               </tr>
